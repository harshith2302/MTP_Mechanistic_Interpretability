...
# --- answer modes (new in pencil_taxonomy_simple) ------------------------------

class _Tok:
    """Stand-in tokenizer: renders messages as plain text so prompt tests need
    no model on disk."""
    def apply_chat_template(self, messages, add_generation_prompt, tokenize):
        return "\n\n".join(f"[{m['role']}]\n{m['content']}" for m in messages) + "\n[assistant]\n"


def _q(qt="person_timestep_lookup"):
    s = generate_story(4, 3, 11)
    return s, [q for q in questions_for_story(s, "x", [qt]) if "question_skipped" not in q][0]


def test_prompt_states_the_rules_of_the_account():
    from src.prompting import build_prompt
    s, q = _q()
    p, _, _ = build_prompt(_Tok(), s["text"], q, True, "direct")
    for phrase in ("chronological order", "AFTER every transfer",
                   "carry forward", "never changes"):
        assert phrase in p, phrase


def test_direct_and_scratchpad_differ_only_in_the_instruction():
    from src.prompting import build_prompt
    s, q = _q()
    d, sha_d, _ = build_prompt(_Tok(), s["text"], q, True, "direct")
    sp, sha_s, _ = build_prompt(_Tok(), s["text"], q, True, "scratchpad")
    assert sha_d == sha_s, "template hash must not depend on answer mode"
    assert "nothing else" in d and "tally" not in d
    assert "tally" in sp and "last line" in sp
    # everything after the instruction (story, question, schema) is identical
    assert d.split("[user]")[1] == sp.split("[user]")[1]
...
def test_unknown_answer_mode_is_rejected():
    ...
    with pytest.raises(ValueError):
        build_prompt(_Tok(), s["text"], q, True, "cot")


def test_scratchpad_budget_grows_with_T_and_direct_does_not():
    from src.prompting import max_tokens_for
    cfg = load_config()
    s, q = _q()
    direct = dict(cfg, generation={**cfg["generation"], "answer_mode": "direct"})
    scratch = dict(cfg, generation={**cfg["generation"], "answer_mode": "scratchpad"})
    few = dict(q, story_meta={"transfers": [(1, "A1", "A2", 1)]})
    many = dict(q, story_meta={"transfers": [(1, "A1", "A2", 1)] * 200})
    assert max_tokens_for(few, direct) == max_tokens_for(many, direct)
    assert max_tokens_for(many, scratch) > max_tokens_for(few, scratch)
    assert max_tokens_for(few, scratch) > max_tokens_for(few, direct)
    # the window cap bites, and never below a usable minimum
    assert max_tokens_for(many, scratch, context_left=500) == 500
    assert max_tokens_for(many, scratch, context_left=3) == 16


def test_scratchpad_records_are_graded_on_the_last_json_object():
    s, q = _q()
    raw = ("t0: 6\nt1: {6} -> 5\nt2: 5\n" + '{"answer": %d}' % q["ground_truth"])
    base = {**q, "raw_output": raw, "truncated": False, "context_overflow": False}
    direct = grade_record({**base, "answer_mode": "direct"})
    scratch = grade_record({**base, "answer_mode": "scratchpad"})
    assert scratch["correct"] and scratch["parse_path"].endswith("last_object")
    # direct mode reads the FIRST object, which here is the reasoning's "{6}"
    assert not direct["correct"] or direct["parse_path"] != scratch["parse_path"]


def test_transfer_recall_only_asks_about_unique_triples():
    """The one sampling rule added over pencil_taxonomy."""
    import collections
    for seed in range(60):
        s = generate_story(2, 1, seed)
        q = questions_for_story(s, "x", ["transfer_recall"])[0]
        if "question_skipped" in q:
            continue
        key = (q["target_timestep"], q["target_person_id"],
               s["name_to_id"][q["target_receiver"]])
        n = collections.Counter((e["timestep"], e["giver"], e["receiver"])
                                for e in s["narration"])[key]
        assert n == 1, "asked about a (t, giver, receiver) that occurs twice"


def test_number_words_are_normalised_but_not_strict():
    """OLMo-2 copies "seven" back verbatim from the story. That is the right
    answer in the wrong form: lenient-correct, strict-wrong, recorded as rescued."""
    from src.parsing import _int
    assert _int("seven") == 7 and _int("Twenty-one") == 21 and _int("one hundred and five") == 105
    assert _int("sevenish") is None and _int("") is None
    s, q = _q("initial_state_lookup")
    from num2words import num2words
    rec = grade_record({**q, "raw_output": '{"answer": "%s"}' % num2words(q["ground_truth"]),
                        "truncated": False, "context_overflow": False, "answer_mode": "direct"})
    assert rec["correct"] and not rec["correct_strict"] and rec["normalisation_rescued"]
