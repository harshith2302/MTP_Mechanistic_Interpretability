...
Three sampling constraints are load-bearing rather than cosmetic:

  * `transfer_recall` only asks about a (timestep, giver, receiver) triple that
    occurs exactly once in the story. With no clusters the same pair can trade
    twice in one timestep -- likely at N=2 -- and "how many did A give B at
    timestep t" would then have no single answer. (The original sampler did not
    check this; it is a correctness fix, recorded as such.)
...
def _answer_space_size(story, pid):
    ...
    The largest count anyone can hold is the total in circulation among the
    people they can trade with -- with no clusters, that is everyone, so the
    space is 0..total_pencils inclusive. This is the range the chance null draws
    from, so it must be the real bound rather than a generous one -- too wide a
    range makes every category look non-coincidental.
    """
...
def q_transfer_recall(story, rng):
    ...
    counts = {}
    for (t, g, r, _) in tr:
        counts[(t, g, r)] = counts.get((t, g, r), 0) + 1
    unique = [e for e in tr if counts[(e[0], e[1], e[2])] == 1]
    if not unique:
        return {"question_skipped": "no (t, giver, receiver) triple occurs exactly once"}
